...
def sanitize_existing_file(path: Path):
    text = path.read_text(encoding="utf-8", errors="replace").translate(SMART_MAP)

    if path.name == "f60_ui_shell.html":
        text = "\n".join(line for line in text.splitlines() if line.strip() != "```") + "\n"

    if path.name == "f61_ui_styles.css" and "--text-2:" not in text:
        text = text.replace(
            ":root {",
            ":root {\n  --text-2: var(--t2);\n  --text-3: var(--t3);\n  --f-mono: var(--f-m);\n  --f-display: var(--f-d);\n",
            1,
...
def main():
    for name in UI_FILES:
        path = ROOT / name
        if path.exists():
            sanitize_existing_file(path)
            print(f"[sanitized] {name}")
        else:
            print(f"[missing] {name}")

    write_text(ROOT / "f64_ui_components.js", F64_UI_COMPONENTS.strip() + "\n")
    print("[rewritten] f64_ui_components.js")

    write_text(ROOT / "f65_ui_theme.js", F65_UI_THEME.strip() + "\n")
    print("[rewritten] f65_ui_theme.js")

    write_text(ROOT / "f67_ui_artifacts.js", F67_UI_ARTIFACTS.strip() + "\n")
    ...
    for file_name in files_to_check:
        path = ROOT / file_name
        if path.exists():
            ok = check_js(file_name) and ok

    print("\nDone.")
    print("Next:")
    print("1. Stop the running app")
    print("2. Run: python nuclear_ui_fix.py")
    print("3. Run: python app.py")
    print("4. Hard refresh the preview/browser")
